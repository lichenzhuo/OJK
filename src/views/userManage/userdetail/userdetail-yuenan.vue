      ...
      <ul class="xuan-2">
        ...
        <li class="xuan-2-1" v-if="active==1">
          <div class="idimgbox">
            ...
            <template>
              <div v-if="userIdcard.idcardPhotoReverseUrl!==null&&userIdcard.idcardPhotoReverseUrl!==undefined&&userIdcard.idcardPhotoReverseUrl!==''" class="idimg pic" @click="openBox({imgUrl:userIdcard.idcardPhotoReverseUrl})">
            ...
            <template>
              <div v-if="userIdcard.idcardFairPhotoUrl!==null&&userIdcard.idcardFairPhotoUrl!==undefined&&userIdcard.idcardFairPhotoUrl!==''" class="idimg pic" @click="openBox({imgUrl:userIdcard.idcardFairPhotoUrl})">
            ...
            <template>
              <div v-if="userIdcard.idcardFairPhotoReverseUrl!==null&&userIdcard.idcardFairPhotoReverseUrl!==undefined&&userIdcard.idcardFairPhotoReverseUrl!==''" class="idimg pic" @click="openBox({imgUrl:userIdcard.idcardFairPhotoReverseUrl})">
        ...
        <li class="xuan-2-2" v-if="active==2">
          <div class="xuan-2-2-1">
            <template>
              <p v-if="userBase.name!==null&&userBase.name!==undefined&&userBase.name!==''"><span>{{$t('public.no1')}}:</span> <span>{{userBase.name}}</span>  </p>
              <p v-else><span>{{$t('public.no1')}}:</span> <span> {{$store.state.common.nullData}} </span> </p>
            </template>
            <template>
              <p v-if="userSelf.degree!==null&&userSelf.degree!==undefined&&userSelf.degree!==''"><span>{{$t('public.no5')}}:</span> <span>{{$t($store.getters.vi_education(userSelf.degree))}}</span> </p>
          ...
          <div class="xuan-2-2-1">
            ...
            <template>
              <p v-if="userSelf.idCard!==null&&userSelf.idCard!==undefined&&userSelf.idCard!==''"><span>{{$t('public.no2')}}:</span> <span>{{userSelf.idCard}}</span>  </p>
              <p v-else><span>{{$t('public.no2')}}:</span> <span> {{$store.state.common.nullData}} </span> </p>
            </template>
            <template>
              <p v-if="userSelf.strIdCardAwardTime!==null&&userSelf.strIdCardAwardTime!==undefined&&userSelf.strIdCardAwardTime!==''">
                <span>{{$t('yuenan.no1')}}:</span> <span>{{userSelf.strIdCardAwardTime}}</span> 
              </p>
          ...
          <div class="xuan-2-2-1">
            ...
            <template>
              <p v-if="userSelf.liveProvinceName!==null&&userSelf.liveProvinceName!==undefined&&userSelf.liveProvinceName!==''"><span>{{$t('yuenan.no2')}}:</span>
                <span>{{userSelf.liveProvinceName}}</span>
              </p>
              <p v-else><span>{{$t('yuenan.no2')}}:</span> <span> {{$store.state.common.nullData}} </span> </p>
            </template>
            <template>
              <p v-if="userSelf.liveCityName!==null&&userSelf.liveCityName!==undefined&&userSelf.liveCityName!==''"><span>{{$t('yuenan.no17')}}:</span>
                <span>{{userSelf.liveCityName}}</span>
              </p>
          ...
          <div class="xuan-2-2-1">
            <template>
              <p v-if="userSelf.liveRegionName!==null&&userSelf.liveRegionName!==undefined&&userSelf.liveRegionName!==''"><span>{{$t('yuenan.no3')}}:</span>
                <span>{{userSelf.liveRegionName}}</span>
              </p>
              <p v-else><span>{{$t('yuenan.no3')}}:</span> <span> {{$store.state.common.nullData}} </span> </p>
            </template>
            <template>
              <p v-if="userSelf.building!==null&&userSelf.building!==undefined&&userSelf.building!==''"><span>{{$t('yuenan.no16')}}:</span>
                <span>{{userSelf.building}}</span>
              </p>
              <p v-else><span>{{$t('yuenan.no16')}}:</span> <span> {{$store.state.common.nullData}} </span> </p>
            </template>
          </div>
        ...
        <li class="xuan-2-3" v-if="active==3">
          <div class="xuan-2-3-1">
            <div class="xuan-2-3-1-1">
              ...
              <template>
                <p v-if="userWork.monthIncome!==null&&userWork.monthIncome!==undefined&&userWork.monthIncome!==''"><span>{{$t('public.no13')}}:</span> <span>{{$store.getters.moneySplit(userWork.monthIncome)}}</span> </p>
                <p v-else><span>{{$t('public.no13')}}:</span> <span> {{$store.state.common.nullData}} </span> </p>
              </template>
            </div>
            <div class="xuan-2-3-1-1">
              <template>
                <p v-if="userWork.industry!==null&&userWork.industry!==undefined&&userWork.industry!==''"><span>{{$t('yuenan.no5')}}:</span> <span>{{userWork.industry}}</span> </p>
            ...
            <div class="xuan-2-3-1-1">
              <template>
                <p v-if="userWork.companyProvinceName!==null&&userWork.companyProvinceName!==undefined&&userWork.companyProvinceName!==''"><span>{{$t('yuenan.no6')}}:</span> <span>{{userWork.companyProvinceName}}</span> </p>
                <p v-else><span>{{$t('yuenan.no6')}}:</span> <span> {{$store.state.common.nullData}} </span> </p>
              </template>
              <template>
                <p v-if="userWork.companyCityName!==null&&userWork.companyCityName!==undefined&&userWork.companyCityName!==''"><span>{{$t('yuenan.no15')}}:</span> <span>{{userWork.companyCityName}}</span> </p>
                <p v-else><span>{{$t('yuenan.no15')}}:</span> <span> {{$store.state.common.nullData}} </span> </p>
              </template>
              <template>
                <p v-if="userWork.companyRegionName!==null&&userWork.companyRegionName!==undefined&&userWork.companyRegionName!==''"><span>{{$t('yuenan.no7')}}:</span> <span>{{userWork.companyRegionName}}</span> </p>
        ...
        <li class="xuan-2-4" v-if="active==4">
          ...
          <div class="xuan-2-4-1" v-if="userUrgentContact.contactTwoRelationName!==null&&userUrgentContact.contactTwoRelationName!==undefined&&userUrgentContact.contactTwoRelationName!==''">
          ...
          <div class="xuan-2-4-1" v-if="userUrgentContact.contactThreeRelationName!==null&&userUrgentContact.contactThreeRelationName!==undefined&&userUrgentContact.contactThreeRelationName!==''">
        ...
        <li class="xuan-2-5" v-if="active==5">
          <table class="back-table" width="100%" border="1" cellspacing="0" cellpadding="20">
            ...
            <template v-if="userBank!==null&&userBank!==undefined&&userBank!=''">
              <tr >
                <template>
                  <td v-if="userBank.bankType!==null&&userBank.bankType!==undefined&&userBank.bankType!==''">{{userBank.bankType}}</td>
                  <td v-else>{{$store.state.common.nullData}}</td>
                </template>
                <template>
                  <td v-if="userBank.bankName!==null&&userBank.bankName!==undefined&&userBank.bankName!==''">{{userBank.bankType==='NganLuong'?'-':userBank.bankName}}</td>
                  <td v-else>{{$store.state.common.nullData}}</td>
                </template>
                <template>
                  <td v-if="userBank.bankAccount!==null&&userBank.bankAccount!==undefined&&userBank.bankAccount!==''">{{userBank.bankType==='NganLuong'?'-':userBank.bankAccount}}</td>
                  <td v-else>{{$store.state.common.nullData}}</td>
                </template>
                <template>
                  <td v-if="userBank.cardFullname!==null&&userBank.cardFullname!==undefined&&userBank.cardFullname!==''">{{userBank.cardFullname}}</td>
                  <td v-else>{{$store.state.common.nullData}}</td>
                </template>
                <template>
                  <td v-if="userBank.email!==null&&userBank.email!==undefined&&userBank.email!==''">{{userBank.email}}</td>
                  <td v-else>{{$store.state.common.nullData}}</td>
                </template>
                <template>
                ...
                <template>
                  <td v-if="userBank.cardYear!==null&&userBank.cardYear!==undefined&&userBank.cardYear!==''">{{userBank.cardYear+'-'+userBank.cardMonth}}</td>
                  <td v-else>{{$store.state.common.nullData}}</td>
                </template>
                <template>
                  <td v-if="userBank.status!==null&&userBank.status!==undefined&&userBank.status!==''">{{userBank.status==1?$t('userDetail.bankId_status.no1'):$t('userDetail.bankId_status.no2')}}</td>
                  <td v-else>{{$store.state.common.nullData}}</td>
                </template>
              </tr>
            </template>
            ...
            <el-table :data="tableData" stripe style="width: 100%" max-height=530>
              ...
              <el-table-column align="center" prop="loanAmount" :label="$t('public.no24')"  min-width="80">
                <template slot-scope="scope">
                  <span v-if="scope.row.loanAmount!==null&&scope.row.loanAmount!==undefined&&scope.row.loanAmount!==''">{{$store.getters.moneySplit(scope.row.loanAmount)}}</span>
              ...
              <el-table-column align="center" prop="returnMoney" :label="$t('public.no27')" min-width="80">
                <template slot-scope="scope">
                  <span v-if="scope.row.returnMoney!==null&&scope.row.returnMoney!==undefined&&scope.row.returnMoney!==''">{{$store.getters.moneySplit(scope.row.returnMoney)}}</span>
                  <span v-else>{{$store.state.common.nullData}}</span>
                </template>
              </el-table-column>
