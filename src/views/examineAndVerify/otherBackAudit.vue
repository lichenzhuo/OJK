  ...
  <div class="public_main">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>{{$t('yn.no36')}}</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <div class="search">
      <el-row type="flex" justify="start" >
        ...
        <div class="search-input ml15" v-if="$store.state.common.permiss.includes('RIGHT_OTHER_REPAYMENT_QUERY')">
    ...
    <!-- -------------表单显示栏------------------------ -->
    <div class="table" v-if="$store.state.common.permiss.includes('RIGHT_OTHER_REPAYMENT_LIST')">
      <template>
        <el-table :data="tableData" size="small" stripe>
          ...
          <el-table-column fixed="right" align="center" prop="operation" :label="$t('public.operation')" min-width="160">
            <template slot-scope="scope">
              <span 
                class="table_opr"
                v-if="$store.state.common.permiss.includes('RIGHT_OTHER_REPAYMENT_REVIEW')&&scope.row.reviewResult===0"
                @click="openAuditDialog(scope.row.orderId,scope.row.userId,scope.row.remark)">
                {{$t('public.no76')}}
              </span>
              <span 
                class="table_opr"
                v-if="$store.state.common.permiss.includes('RIGHT_OTHER_REPAYMENT_DETAIL')"
    ...
    <el-row type="flex" justify="end" >
      <div class="pages" v-if="$store.state.common.permiss.includes('RIGHT_OTHER_REPAYMENT_LIST')">
        <el-pagination
...
export default {
  ...
  methods: {
    ...
    jumpDetali (orderNo,orderId) { // 查看详情
      this.$router.push({path: '/cuishoudetail', query: {orderNo, orderId, type: '2', from: '1'}});
    },
    getTableList () { // 列表数据
      let option = {
        header: {
