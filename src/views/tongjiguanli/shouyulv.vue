<template>
  <div class="usermanage">
    ...
    <div class="table">
      <template>
        <el-table :data="tableData" size="small" style="width:100%" empty-text>
          <el-table-column align="center" prop="strLoanTime" :label="$t('public.no58')" width="180">
    ...
    <el-table :data="tableData1" :row-style="rowStyle" :show-header="false" size="small" :row-class-name="tableRowClassName" style="width: 100%">
      ...
      <el-table-column align="center" prop="loanCount" :label="$t('totalManage.applyCount')" min-width="100">
      </el-table-column>
      <el-table-column align="center" prop="loanAmount" :label="$t('totalManage.machineCount')" min-width="100">
        <template slot-scope="scope">
          <span v-if="scope.row.loanAmount!==null&&scope.row.loanAmount!==undefined&&scope.row.loanAmount!==''">{{$store.state.common.id_currency}}{{$store.getters.moneySplit(scope.row.loanAmount)}}{{$store.state.common.vi_currency}}</span>
          <span v-else>{{$store.state.common.nullData}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="refundCount" :label="$t('totalManage.firstCount')" min-width="100">
      </el-table-column>
      <el-table-column align="center" prop="refundAmount" :label="$t('totalManage.reviewCount')" min-width="100">
        <template slot-scope="scope">
          <span v-if="scope.row.refundAmount!==null&&scope.row.refundAmount!==undefined&&scope.row.refundAmount!==''">{{$store.state.common.id_currency}}{{$store.getters.moneySplit(scope.row.refundAmount)}}{{$store.state.common.vi_currency}}</span>
          <span v-else>{{$store.state.common.nullData}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="moneyRate" :label="$t('totalManage.laveCount')" min-width="100">
        <template slot-scope="scope">
          <span v-if="scope.row.moneyRate!==null&&scope.row.moneyRate!==undefined&&scope.row.moneyRate!==''">{{$store.getters.twoPoint(scope.row.moneyRate)}}%</span>
          <span v-else>{{$store.state.common.nullData}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="countRate" :label="$t('totalManage.laveCount')" min-width="80">
        <template slot-scope="scope">
          <span v-if="scope.row.countRate!==null&&scope.row.countRate!==undefined&&scope.row.countRate!==''">{{$store.getters.twoPoint(scope.row.countRate)}}%</span>
          <span v-else>{{$store.state.common.nullData}}</span>
        </template>
    ...
    formList () { // 获取首逾率列表
      ...
      this.$axios.post('',option).then(res=>{
        ...
        if(res.data.header.code==0){
          ...
          this.tableData1.push(res.data.data.overAll);
          this.pageTotal = res.data.header.page.total;
        }
      })
    },
    select () { // 搜索栏查询按钮点击操作
      this.$store.commit('shouyulv', this.formInline)
      if (this.flag) {
        this.flag = false
        this.formList();
      }
