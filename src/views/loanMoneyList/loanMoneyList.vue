  ...
  <div class="usermanage">
    ...
    <div class="search">
      <el-row type="flex" justify="start" :gutter="10">
        ...
        <template v-if="$store.state.common.permiss.includes('RIGHT_LOAN_LIST_EXP')">
        ...
        <el-table :data="tableData" size="small" stripe empty-text>
          ...
          <template v-if="$store.state.common.lang==='vi'">
            ...
            <el-table-column align="center" prop="orderLoanType" :label="$t('websiteLoans.no5')" min-width="90">
              <template slot-scope="scope">
                <span v-if="scope.row.problemType!==null&&scope.row.orderLoanType!==undefined&&scope.row.orderLoanType!==''">{{$t($store.getters.loanTypeState(scope.row.orderLoanType))}}</span>
          ...
          <template v-if="$store.state.common.lang==='id'">
            <el-table-column align="center" prop="problemType" :label="$t('yn.no4')" min-width="120">
              <template slot-scope="scope">
                <span v-if="scope.row.problemType!=''">{{$t($store.getters.handlingProblem1(scope.row.problemType))}}</span>
                <span v-else>{{$store.state.common.nullData}}</span>
              </template>
            </el-table-column>
            <el-table-column align="center" prop="dealStatus" :label="$t('yn.no5')" min-width="90">
              <template slot-scope="scope">
                <span v-if="scope.row.dealStatus!=''">{{$t($store.getters.handlingStatus(scope.row.dealStatus))}}</span>
                <span v-else>{{$store.state.common.nullData}}</span>
              </template>
            </el-table-column>
          </template>
          ...
          <el-table-column fixed="right" align="center" prop="operation" :label="$t('public.operation')" width="180">
            <template slot-scope="scope">
              <span 
                v-if="$store.state.common.permiss.includes('RIGHT_LOAN_LIST_SHOW')"
                class="mg5"
              ...
              <span
                v-if="$store.state.common.permiss.includes('RIGHT_LOAN_LIST_DOWN')" 
                class="mg5"
                style="color:#547ef6;cursor:pointer" 
              ...
              <template v-if="$store.state.common.permiss.includes('RIGHT_LOAN_LIST_DEAL')">
                <span
                  v-if="$store.state.common.lang==='id'&&(scope.row.status === 43||scope.row.status === 50)&&scope.row.dealStatus==0"
                  class="mg5"
                  style="color:#547ef6;cursor:pointer" 
