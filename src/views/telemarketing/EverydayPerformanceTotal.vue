  ...
  <div class="public_main">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>{{$t('teleMarketing.no4')}}</el-breadcrumb-item>
      </el-breadcrumb>
    </div>

    <div class="paixu">
      <span></span>
      <p>{{$t('teleMarketing.no4')}}</p>
    </div>
    ...
    <div class="search">
      <el-row type="flex" justify="start" :gutter="10">
        ...
        <template v-if="$store.state.common.permiss.includes('RIGHT_PHONE_MARKET_PERFORMANCE_QUERY')">
          <div class="search-input ml15">
            <el-button type="primary" class="button-color" @click="select">{{$t('public.select')}}</el-button>
          </div>
        </template>
        <template v-if="$store.state.common.permiss.includes('RIGHT_PHONE_MARKET_PERFORMANCE_EXP')">
          <div class="search-input ml15">
    ...
    <!-- -------------表单显示栏------------------------ -->
    <div class="table" v-if="$store.state.common.permiss.includes('RIGHT_PHONE_MARKET_PERFORMANCE_LIST')">
      <template>
        <el-table :data="tableData" size="small" >
          <el-table-column align="center" prop="strTime" :label="$t('public.no33')" width="140">
          ...
          <el-table-column align="center" prop="followCountTotal" :label="$t('teleMarketing.no24')">
          </el-table-column>
          <el-table-column align="center" prop="applyCount" :label="$t('teleMarketing.no25')">
          </el-table-column>
          <el-table-column align="center" prop="loanCount" :label="$t('teleMarketing.no26')">
          </el-table-column>
        </el-table>
      </template>
    </div>
    ...
    <el-row type="flex" justify="end">
      <div class="pages" v-if="$store.state.common.permiss.includes('RIGHT_PHONE_MARKET_PERFORMANCE_LIST')">
        <el-pagination
  ...
  methods: {
    ...
    getTableData () { // 获取列表
      let option = {
        header: {
          ...this.$base,
          action: this.$store.state.actionMap.PHONEMARKETPERFM0001,
    ...
    putExcel () { // excel下载操作
      if (this.flag) {
        ...
        let option = {
          header: {
            ...this.$base,
            action: this.$store.state.actionMap.PHONEMARKETPERFM0002,
            'page': {'index': this.currentPage, 'size': 10},
            'sessionid': this.sessionid
          },
          ...this.formInline
