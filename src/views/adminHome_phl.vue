...
export default {
  ...
  computed: {
    lang1 () { // 还款标题
      return this.$t('adminHome.no13')
    },
    lang2 () { // 图表鼠标移入显示数量
      return this.$t('adminHome.number')
    },
    lang3 () { // 本月标题
      let now = new Date()
      let yearn  = now.getFullYear();
      let month = now.getMonth()+1;
      return yearn + '-' + month + this.$t('adminHome.no14')
    },
    week () { // 星期国际化
    ...
    desc () { // 标注国际化
      return [
          this.$t('adminHome.todyRegisterNum'), 
          this.$t('adminHome.no17'), 
  ...
  methods: {
    dataList () { // 获取页面数据
      let option = {
        header: {
          ...this.$base,
          action: this.$store.state.actionMap.admin_home,
          'sessionid': this.sessionid
        }
      }
      this.$axios.post('', option).then(res => {
        if (res.data.header.code == 0) {
          this.mainData.all = res.data.data.all
          this.mainData.amount = res.data.data.amount
    ...
    drawLine1 () {    // 本周每日还款量统计图
      if (this.data1) {
        ...
        var colors = ['#4FC913', '#AD98D9', '#50A8EC', '#FFB075', '#D26F75', '#828DAA', '#E1C810']
        let obj = {};
        ...
        myChart.setOption({
          ...
          series: [
              {
                  name:this.$t('adminHome.todyRegisterNum'),
                  type:'line',
                  smooth: true,
                  data:this.data1.registerCountWeek
              },
              {
                  name:this.$t('adminHome.no17'),
                  type:'line',
                  smooth: true,
                  data:this.data1.applyTodayCountWeek
              },
              {
              ...
              },
              {
                  ...
                  type:'line',
                  smooth: true,
                  data:this.data1.oldApplyCountWeek
              },
              {
                  name:this.$t('adminHome.no19'),
                  type:'line',
                  smooth: true,
              ...
              {
                  name:this.$t('adminHome.no18'),
                  type:'line',
                  smooth: true,
                  data:this.data1.lendingTodayCountWeek
              },
              {
                  name:this.$t('adminHome.no9'),
                  type:'line',
                  smooth: true,
                  data:this.data1.newLendCountWeek
              },
              {
                  name:this.$t('adminHome.no15'),
                  type:'line',
                  smooth: true,
                  data:this.data3.repaymentWeek
              },
              {
                  name:this.$t('adminHome.no16'),
                  type:'line',
                  smooth: true,
                  data:this.data1.overDueWeek
              },
          ]
        })
      }
          ...
          series: [
              {
                  ...
                  type:'line',
                  smooth: true,
                  data:this.data2.registerCountMonth
              },
              {
                  name:this.$t('adminHome.no17'),
                  type:'line',
                  smooth: true,
                  data:this.data2.applyTodayCountMonth
              },
              {
                  name:this.$t('adminHome.no7'),
                  type:'line',
              ...
              },
              {
                  ...
                  type:'line',
                  smooth: true,
                  data:this.data2.oldApplyCountMonth
              },
              {
                  name:this.$t('adminHome.no19'),
                  type:'line',
              ...
              },
              {
                  ...
                  type:'line',
                  smooth: true,
                  data:this.data2.lendingTodayCountMonth
              },
              {
                  name:this.$t('adminHome.no9'),
                  type:'line',
                  smooth: true,
                  data:this.data2.newLendCountMonth
              },
              {
                  name:this.$t('adminHome.no15'),
                  type:'line',
