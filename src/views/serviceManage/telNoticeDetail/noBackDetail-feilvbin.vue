<template>
  <div v-if="data" class="usermanage">
    ...
    <div class="xuan-2-5">
      <table class="bank-table" width="100%" border="1" cellspacing="0" cellpadding="20">
        <tr>
          <th width="15%">{{$t('staffManage.xinshenyuan')}}</th>
          <th width="15%">{{$t('new.no30')}}</th>
          <th width="15%">{{$t('new.no9')}}</th>
          <th width="55%">{{$t('new.no31')}}</th>
        </tr>
        <template v-if="(data.orderPhoneApproveList!==null&&data.orderPhoneApproveList!==undefined&&data.orderPhoneApproveList!='')||(data.orderLendingList!==null&&data.orderLendingList!==undefined&&data.orderLendingList!='')||(data.orderRepaymentList!==null&&data.orderRepaymentList!==undefined&&data.orderRepaymentList!='')">
          <template v-if="data.orderPhoneApproveList!==null&&data.orderPhoneApproveList!==undefined&&data.orderPhoneApproveList!=''">
            <tr v-for="value in data.orderPhoneApproveList" :key="value.id">
              <template>
                <td v-if="value.adminName!==null&&value.adminName!==undefined">{{value.adminName}}</td>
                <td v-else>{{$store.state.common.nullData}}</td>
              </template>
              <template>
                <td v-if="value.approveStage!==null&&value.approveStage!==undefined">{{value.approveStage==1?$t('myAuditList.no5'):$t('myAuditList.no9')}}</td>
                <td v-else>{{$store.state.common.nullData}}</td>
              </template>
              <template>
    ...
    detail () {
      ...
      this.$axios.post('', option).then(res => {
        if (res.data.header.code == 0) {
          ...
          this.data.orderPhoneApproveList = res.data.data.orderPhoneApproveList
