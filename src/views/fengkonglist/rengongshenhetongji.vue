<template>
  <div class="usermanage">
    ...
    <div class="search">
      <el-row type="flex" :gutter="20" justify="start">
        ...
        <el-col :md="9" :lg="6" :xl="5">
          <div class="search-input">
            <span>{{$t('loanMoneyDetail.opeType2')}}:</span>
    ...
    <div class="table" v-if="$store.state.common.permiss.includes('RIGHT_RISKCONTROL_RPT_LIST')">
      <template>
        <el-table ref="multipleTable" size="small" :data="tableData" stripe  empty-text >
          ...
          <el-table-column align="center" prop="failCount" :label="$t('riskManage.failCount')" min-width="120">
          </el-table-column>
...
export default {
  ...
  data () {
    return {
      ...
      currentPage: 1, // 当前页下标
      options1: this.$store.state.options.auditTotal_option, // 认证状态下拉选框信息
      tableData: []// 列表数据模拟
    }
  },
  methods: {
    ...
    select () { // 点击查询按钮操作
      this.$store.commit('rengongtongjiList', this.formInline)
      if (this.flag) {
        this.flag = false
        this.tongji();
      }
  ...
  mounted () {
    ...
    if (JSON.stringify(this.$store.state.common.rengongtongji_select) !== '{}') {
      this.formInline = this.$store.state.common.rengongtongji_select
