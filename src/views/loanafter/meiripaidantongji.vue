<template>
  <div class="usermanage">
    ...
    <div class="table" v-if="$store.state.common.permiss.includes('RIGHT_DISTRIBUTION_STATISTICS_LIST')">
      <template>
        <el-table :data="tableData" stripe style="width: 100%" empty-text>
          ...
          <el-table-column align="center" prop="adminName" :label="$t('loanAfterManage.name')" min-width="120">
          </el-table-column>
          <el-table-column align="center" prop="roleName" :label="$t('loanAfterManage.type1')" min-width="120">
          </el-table-column>
          ...
          <el-table-column align="center" prop="number" :label="$t('loanAfterManage.fenpeidanliang')" min-width="90">
          </el-table-column>
          <el-table-column align="center" prop="transferNumber" :label="$t('loanAfterManage.zhuanpailiang')" min-width="90">
          </el-table-column>
          <el-table-column align="center" prop="money" :label="$t('loanAfterManage.yinghuanbenxi')" min-width="100">
          ...
          <el-table-column align="center" prop="operation" :label="$t('public.operation')" min-width="120">
            <template slot-scope="scope">
              <span style="color:#547ef6;cursor:pointer" @click="detail(scope.row.adminId,scope.row.time,scope.row.roleName,scope.row.groupId)">{{$t('loanAfterManage.sel')}}</span>
              <template v-if="$store.state.common.permiss.includes('RIGHT_DISTRIBUTION_STATISTICS_SHIFT')">
                <span 
                  ...
                  style="color:#547ef6;cursor:pointer;margin-left:10px;" 
        ...
        <el-table :data="tableData1" :row-style="rowStyle" :row-class-name="tableRowClassName" :show-header="false" size="small" style="width: 100%">
          ...
          <el-table-column align="center" prop="allocationQuantity" :label="$t('loanAfterManage.fenpeidanliang')" min-width="90">
          </el-table-column>
          <el-table-column align="center" prop="transferOrder" :label="$t('loanAfterManage.zhuanpailiang')" min-width="90">
          </el-table-column>
          <el-table-column align="center" prop="payPrincipalInterest" :label="$t('loanAfterManage.yinghuanbenxi')" min-width="100">
            <template slot-scope="scope">
              <span v-if="scope.row.payPrincipalInterest!==null&&scope.row.payPrincipalInterest!==undefined&&scope.row.payPrincipalInterest!==''">{{$store.state.common.id_currency}}{{$store.getters.moneySplit(scope.row.payPrincipalInterest)}}{{$store.state.common.vi_currency}}</span>
              <span v-else>{{$store.state.common.nullData}}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="operation" :label="$t('public.operation')" min-width="120">
    ...
    <!-- ------------------ 点击查看详情弹窗开始 -------------------- -->
    <div v-if="detailFlag" class="detail1">
      <div class="detail-main">
        <div class="detail-main-head">
          <span></span>
          <p>{{$t('loanAfterManage.sel')}}</p>
          <i class="el-icon-shop-guanbi icon-color" style="cursor:pointer" @click="detailFlag=false"></i>
        </div>
    ...
    statisticsList () { // 获取催回统计列表
      ...
      this.$axios.post('', option).then(res => {
        ...
        if (res.data.header.code == 0) {
          ...
          this.tableData1.push(res.data.data.pieceSum);
